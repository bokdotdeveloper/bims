...
const monthlyChartOptions = computed(() => ({
    chart: { type: 'area', toolbar: { show: false }, sparkline: { enabled: false }, background: chartBackground.value },
    theme: chartTheme.value,
    stroke: { curve: 'smooth', width: 2 },
    fill: { type: 'gradient', gradient: { shadeIntensity: 1, opacityFrom: 0.4, opacityTo: 0.05 } },
    xaxis: { categories: props.charts.monthlyAssistance.map(m => m.month), labels: { style: { fontSize: '10px' } } },
    yaxis: { labels: { formatter: (v: number) => '₱' + (v / 1000).toFixed(0) + 'k', style: { fontSize: '10px' } } },
    tooltip: { y: { formatter: (v: number) => formatPeso(v) } },
    colors: ['#1e40af'],
...
const byTypeOptions = computed(() => ({
    chart: { type: 'bar', toolbar: { show: false }, background: chartBackground.value },
    theme: chartTheme.value,
    plotOptions: { bar: { horizontal: true, borderRadius: 4 } },
    xaxis: { categories: props.charts.assistanceByType.map(a => a.assistance_type), labels: { style: { fontSize: '10px' } } },
...
const sexChartOptions = computed(() => ({
    chart: { type: 'donut', background: chartBackground.value },
    theme: chartTheme.value,
...
const barangayOptions = computed(() => ({
    ...
    xaxis: { categories: props.charts.topBarangays.map(b => b.barangay), labels: { style: { fontSize: '9px' } } },
    colors: ['#7c3aed'],
    dataLabels: { enabled: false },
    grid: { borderColor: isDark.value ? '#374151' : '#f3f4f6' },
}));
        ...
        <div class="py-6">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
                    ...
                    <apexchart type="area" height="200" :options="monthlyChartOptions" :series="monthlySeries" />
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-4 md:col-span-1">
                        ...
                        <apexchart type="donut" height="220" :options="sexChartOptions" :series="sexSeries" />
                    </div>
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-4 md:col-span-1">
                        <div class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">By Civil Status</div>
                        <apexchart type="donut" height="220" :options="civilChartOptions" :series="civilSeries" />
                    </div>
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-4 md:col-span-1">
                        <div class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">Top 5 Barangays</div>
                        <apexchart type="bar" height="220" :options="barangayOptions" :series="barangaySeries" />
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
                        ...
                        <apexchart type="bar" height="250" :options="byTypeOptions" :series="byTypeSeries" />
                    </div>
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
